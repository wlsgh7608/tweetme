...
class TweetTestCase(TestCase):
    def setUp(self):
        ...
        # User.objects.create_user(username = 'asd',password = 'somepassword')
        self.user = User.objects.create_user(username = 'qwe',password = 'somepassword')
        self.userb = User.objects.create_user(username = 'qwe22',password = 'somepassword22')
    ...
    def test_action_like(self):
        ...
        response = client.post(
            "/api/tweets/action",{"id":1,"action":"like"})
    ...
    def test_action_unlike(self):
        client = self.get_client()
        response = client.post(
            "/api/tweets/action",{"id":2,"action":"unlike"})
        self.assertEqual(response.status_code,200)
        response = client.post(
            "/api/tweets/action",{"id":2,"action":"unlike"})
        self.assertEqual(response.status_code,200)
    ...
    def test_action_retweet(self):
        ...
        response = client.post(
            "/api/tweets/action",{"id":2,"action":"retweet"})
    ...
    def test_tweet_delete_api_view(self):
        ...
        self.assertEqual(response_incorrect_owner.status_code , 404) # why not 401???? error.....
